...
// or nfancyBox Commercial License for commercial use
//
// http://fancyapps.com/nfancybox/
...
(function (window, document, $, undefined) {
  ...
  // If there's no jQuery, nfancyBox can't work
  // =========================================
  ...
  // Check if nfancyBox is already initialized
  // ========================================
  ...
  var defaults = {
    ...
    // and they will be placed into toolbar (class="nfancybox-toolbar"` element)
    buttons: [
      "zoom",
      //"share",
      "slideShow",
    ...
    */
    ...
    thumbs: {
      ...
      parentEl: ".nfancybox-container", // Container is injected into this element
    ...
    */
    ...
    // Use options below to customize taken action when user clicks or double clicks on the nfancyBox area,
  ...
  $.extend(nFancyBox.prototype, {
    ...
    init: function () {
      ...
      if (
        !$.nfancybox.getInstance() &&
        firstItemOpts.hideScrollbar !== false &&
        !$.nfancybox.isMobile &&
        document.body.scrollHeight > window.innerHeight
      ) {
        $("head").append(
          '<style id="nfancybox-style-noscroll" type="text/css">.compensate-for-scrollbar{margin-right:' +
    ...
    addContent: function (content) {
      ...
      $.each(items, function (i, item) {
        ...
        if ($.isPlainObject(item)) {
          ...
          // $.nfancybox.open( [ { src : "image.jpg", type : "image" } ] )
          ...
          //   $.nfancybox.open( '<div><h1>Hi!</h1></div>' );
          obj = {
            type: "html",
            src: item + ""
          };
    ...
    setContent: function (slide, content) {
      ...
      if (isQuery(content) && content.parent().length) {
        // Make sure content is not already moved to nfancyBox
    ...
    revealContent: function (slide) {
      ...
      //effectClassName = "nfancybox-animated nfancybox-slide--" + (slide.pos >= self.prevPos ? "next" : "previous") + " nfancybox-fx-" + effect;
      effectClassName = "nfancybox-slide--" + (slide.pos >= self.prevPos ? "next" : "previous") + " nfancybox-animated nfancybox-fx-" + effect;

      $slide.addClass(effectClassName).removeClass("nfancybox-slide--current"); //.addClass(effectClassName);
    ...
    focus: function (e, firstRun) {
      ...
      if (e || !self.current || !self.current.isComplete) {
        // Focus on any element inside nfancybox
    ...
    close: function (e, d) {
      ...
      if (effect && duration) {
        $.nfancybox.animate(
          current.$slide.addClass("nfancybox-slide--previous").removeClass("nfancybox-slide--current"),
  ...
  $.nfancybox = {
    ...
    //
    //   $instance = $.nfancybox.getInstance();
    //   $.nfancybox.getInstance().jumpTo( 1 );
    //   $.nfancybox.getInstance( 'jumpTo', 1 );
    //   $.nfancybox.getInstance( function() {
  ...
  // Default click handler for "nfancyboxed" links
  // ============================================
  ...
  // Self initializing plugin for all elements having `data-nfancybox` attribute
  // ==========================================================================
...
(function (window, document, $) {
  ...
  var isClickable = function ($el) {
    ...
    // Check for attributes like data-nfancybox-next or data-nfancybox-close
...
//
// Example of usage:
// $.nfancybox.getInstance().SlideShow.start()
//
// ==========================================================================
(function (document, $) {
  "use strict";
...
(function (window, document, $) {
  ...
  // Trigger click evnt on links to open new nfancyBox instance
  function triggerFromUrl(url) {
    if (url.gallery !== "") {
      // If we can find element matching 'data-nfancybox' atribute,
      // then triggering click event should start nfancyBox
  ...
  $(function () {
    ...
    // Update hash when opening/closing nfancyBox
    $(document).on({
      "onInit.fb": function (e, instance) {
        var url, gallery;
      ...
      "beforeShow.fb": function (e, instance, current, firstRun) {
        ...
        // Variable containing last hash value set by nfancyBox
        // It will be used to determine if nfancyBox needs to close after hash change is detected
    ...
    $(window).on("hashchange.fb", function () {
      ...
      // Find last nfancyBox instance that has "hash"
      $.each(
        $(".nfancybox-container")
        .get()
        .reverse(),
    ...
    // Check current hash and trigger click event on matching element to start nfancyBox, if needed
    setTimeout(function () {
